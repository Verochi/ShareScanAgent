package com.alipay.sdk.m.x;

/* loaded from: classes.dex */
public class a {
    public static final java.lang.String i = "正在加载";
    public static final java.lang.String j = "去支付宝付款";
    public static final java.lang.String k = "去支付宝授权";
    public static java.lang.String l = "";
    public com.alipay.sdk.m.x.a.d a;
    public android.app.Activity b;
    public java.lang.String c;
    public long d = -1;
    public final int e = 1;
    public final long f = 10000;
    public boolean g = false;
    public android.os.Handler h = new com.alipay.sdk.m.x.a.c(android.os.Looper.getMainLooper());

    /* renamed from: com.alipay.sdk.m.x.a$a, reason: collision with other inner class name */
    public class RunnableC0047a implements java.lang.Runnable {
        public RunnableC0047a() {
        }

        @Override // java.lang.Runnable
        public void run() {
            if (com.alipay.sdk.m.x.a.this.a == null) {
                com.alipay.sdk.m.x.a aVar = com.alipay.sdk.m.x.a.this;
                com.alipay.sdk.m.x.a aVar2 = com.alipay.sdk.m.x.a.this;
                aVar.a = aVar2.new d(aVar2.b);
                com.alipay.sdk.m.x.a.this.a.setCancelable(com.alipay.sdk.m.x.a.this.g);
            }
            try {
                if (com.alipay.sdk.m.x.a.this.a.isShowing()) {
                    return;
                }
                com.alipay.sdk.m.x.a.this.a.show();
                com.alipay.sdk.m.x.a.this.h.sendEmptyMessageDelayed(1, 10000L);
            } catch (java.lang.Exception e) {
                com.alipay.sdk.m.u.e.a(e);
            }
        }
    }

    public class b implements java.lang.Runnable {
        public b() {
        }

        @Override // java.lang.Runnable
        public void run() {
            if (com.alipay.sdk.m.x.a.this.a == null || !com.alipay.sdk.m.x.a.this.a.isShowing()) {
                return;
            }
            try {
                com.alipay.sdk.m.x.a.this.h.removeMessages(1);
                com.alipay.sdk.m.x.a.this.a.dismiss();
            } catch (java.lang.Exception e) {
                com.alipay.sdk.m.u.e.a(e);
            }
        }
    }

    public class c extends android.os.Handler {
        public c(android.os.Looper looper) {
            super(looper);
        }

        @Override // android.os.Handler
        public void dispatchMessage(android.os.Message message) {
            com.alipay.sdk.m.x.a.this.a();
        }
    }

    public class d extends android.app.AlertDialog {
        public d(android.content.Context context) {
            super(context);
        }

        private int a(android.content.Context context, float f) {
            return (int) (f * (context == null ? android.content.res.Resources.getSystem() : context.getResources()).getDisplayMetrics().density);
        }

        private android.graphics.drawable.Drawable a(android.content.Context context, java.lang.String str, int i) {
            java.io.ByteArrayInputStream byteArrayInputStream;
            try {
                byteArrayInputStream = new java.io.ByteArrayInputStream(com.alipay.sdk.m.n.a.a(str));
                try {
                    android.graphics.BitmapFactory.Options options = new android.graphics.BitmapFactory.Options();
                    if (i <= 0) {
                        i = 240;
                    }
                    options.inDensity = i;
                    options.inTargetDensity = context.getResources().getDisplayMetrics().densityDpi;
                    android.graphics.drawable.BitmapDrawable bitmapDrawable = new android.graphics.drawable.BitmapDrawable(context.getResources(), android.graphics.BitmapFactory.decodeStream(byteArrayInputStream, null, options));
                    try {
                        byteArrayInputStream.close();
                    } catch (java.lang.Exception unused) {
                    }
                    return bitmapDrawable;
                } catch (java.lang.Throwable th) {
                    th = th;
                    try {
                        com.alipay.sdk.m.u.e.a(th);
                        if (byteArrayInputStream == null) {
                            return null;
                        }
                        try {
                            byteArrayInputStream.close();
                            return null;
                        } catch (java.lang.Exception unused2) {
                            return null;
                        }
                    } catch (java.lang.Throwable th2) {
                        if (byteArrayInputStream != null) {
                            try {
                                byteArrayInputStream.close();
                            } catch (java.lang.Exception unused3) {
                            }
                        }
                        throw th2;
                    }
                }
            } catch (java.lang.Throwable th3) {
                th = th3;
                byteArrayInputStream = null;
            }
        }

        private android.view.View a(android.content.Context context) {
            android.widget.LinearLayout linearLayout = new android.widget.LinearLayout(context);
            android.widget.FrameLayout.LayoutParams layoutParams = new android.widget.FrameLayout.LayoutParams(-2, a(context, 50.0f));
            layoutParams.gravity = 17;
            linearLayout.setOrientation(0);
            linearLayout.setLayoutParams(layoutParams);
            android.graphics.drawable.GradientDrawable gradientDrawable = new android.graphics.drawable.GradientDrawable();
            gradientDrawable.setColor(-450944201);
            gradientDrawable.setCornerRadius(a(context, 5.0f));
            linearLayout.setBackgroundDrawable(gradientDrawable);
            android.widget.ImageView imageView = new android.widget.ImageView(context);
            android.widget.LinearLayout.LayoutParams layoutParams2 = new android.widget.LinearLayout.LayoutParams(a(context, 20.0f), a(context, 20.0f));
            layoutParams2.gravity = 16;
            layoutParams2.setMargins(a(com.alipay.sdk.m.x.a.this.b, 17.0f), a(com.alipay.sdk.m.x.a.this.b, 10.0f), a(com.alipay.sdk.m.x.a.this.b, 8.0f), a(com.alipay.sdk.m.x.a.this.b, 10.0f));
            imageView.setLayoutParams(layoutParams2);
            imageView.setScaleType(android.widget.ImageView.ScaleType.FIT_CENTER);
            imageView.setImageDrawable(a(context, com.alipay.sdk.m.x.a.l, com.autonavi.amap.mapcore.tools.GlMapUtil.DEVICE_DISPLAY_DPI_XHIGH));
            android.view.animation.RotateAnimation rotateAnimation = new android.view.animation.RotateAnimation(0.0f, 359.0f, 1, 0.5f, 1, 0.5f);
            rotateAnimation.setRepeatCount(-1);
            rotateAnimation.setDuration(900L);
            rotateAnimation.setInterpolator(new android.view.animation.LinearInterpolator());
            imageView.startAnimation(rotateAnimation);
            android.widget.TextView textView = new android.widget.TextView(context);
            textView.setText(android.text.TextUtils.isEmpty(com.alipay.sdk.m.x.a.this.c) ? com.alipay.sdk.m.x.a.i : com.alipay.sdk.m.x.a.this.c);
            textView.setTextSize(16.0f);
            textView.setTextColor(-1);
            android.widget.LinearLayout.LayoutParams layoutParams3 = new android.widget.LinearLayout.LayoutParams(-2, -2);
            layoutParams3.gravity = 16;
            layoutParams3.setMargins(0, 0, a(context, 17.0f), 0);
            textView.setLayoutParams(layoutParams3);
            linearLayout.addView(imageView);
            linearLayout.addView(textView);
            android.widget.FrameLayout frameLayout = new android.widget.FrameLayout(context);
            frameLayout.setLayoutParams(new android.widget.FrameLayout.LayoutParams(-2, -2, 17));
            frameLayout.addView(linearLayout);
            return frameLayout;
        }

        @Override // android.app.AlertDialog, android.app.Dialog
        public void onCreate(android.os.Bundle bundle) {
            super.onCreate(bundle);
            setContentView(a(getContext()));
            android.view.Window window = getWindow();
            if (window != null) {
                window.setBackgroundDrawable(new android.graphics.drawable.ColorDrawable(0));
            }
        }
    }

    public a(android.app.Activity activity) {
        this.b = activity;
    }

    public a(android.app.Activity activity, java.lang.String str) {
        this.b = activity;
        this.c = str;
    }

    public void a() {
        android.app.Activity activity = this.b;
        if (activity != null) {
            activity.runOnUiThread(new com.alipay.sdk.m.x.a.b());
        }
    }

    public void a(java.lang.String str) {
        this.c = str;
    }

    public void a(boolean z) {
        this.g = z;
    }

    public void b() {
        this.b = null;
        this.a = null;
    }

    public java.lang.String c() {
        return this.c;
    }

    public void d() {
        android.app.Activity activity = this.b;
        if (activity != null) {
            activity.runOnUiThread(new com.alipay.sdk.m.x.a.RunnableC0047a());
        }
    }
}
